    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto relative">
        <header className="flex items-center justify-between px-8 py-4 bg-white border-b border-slate-200 sticky top-0 z-20">
          <h1 className="font-bold text-lg text-slate-800">Pathways</h1>
        ...
        <div className="p-8 space-y-6 max-w-[1400px] mx-auto">
          ...
          <div className="flex items-center justify-between">
             <span className="text-sm font-bold text-slate-700">0 pathways</span>
          ...
          <div className="flex flex-col items-center justify-center pt-32 pb-20 text-center">
             <div className="relative mb-8">
                ...
                <div className="w-40 h-28 bg-white rounded-2xl shadow-xl border border-slate-100 relative flex items-center justify-center translate-y-[-10px] overflow-hidden">
                   {/* Pathway Map Illustration */}
                   <div className="relative w-full h-full p-4 flex flex-col justify-between">
                      <div className="flex justify-start">
                         <div className="w-8 h-8 rounded-full border-2 border-blue-100 flex items-center justify-center bg-white shadow-sm">
                            <div className="w-3 h-3 rounded-full bg-blue-200" />
                         </div>
                      </div>
                      
                      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                         <Route size={32} className="text-blue-100 stroke-[1.5]" />
                      </div>

                      <div className="flex justify-end relative">
                         <div className="w-10 h-10 rounded-full border-2 border-blue-500 flex items-center justify-center bg-blue-50 shadow-md">
                            <Flag size={18} className="text-blue-600" />
                         </div>
                         {/* Connector Dash Line */}
                         <div className="absolute bottom-1/2 right-12 w-16 border-t-2 border-dashed border-blue-100 -translate-y-[-10px] rotate-[-20deg]" />
                         <div className="absolute top-1/4 left-0 w-16 border-t-2 border-dashed border-blue-100 -translate-x-[110px] translate-y-[15px] rotate-[20deg]" />
                      </div>
                   </div>

                   {/* Floating Badge Overlay */}
                   <div className="absolute -top-4 -right-4 w-12 h-12 bg-white rounded-xl shadow-lg border border-slate-50 flex items-center justify-center animate-bounce duration-[3000ms]">
                      <CheckCircle2 size={24} className="text-emerald-500" />
                   </div>
                </div>
             </div>
             
             <h2 className="text-xl font-extrabold text-slate-800 mb-4 font-display">You have no pathways yet</h2>
             <p className="text-slate-400 text-sm max-w-sm mb-8 leading-relaxed font-medium">
                Pathways allow you to create structured learning journeys that automatically issue credentials as learners meet specific milestones or complete multiple courses.
             </p>
